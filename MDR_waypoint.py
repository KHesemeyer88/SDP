...
def haversine_distance(lat1, lon1, lat2, lon2):
    R = 6371.137  # Radius of the Earth in kilometers
...
while True:
    ...
    if ser_bytes: # skip empty lines from serial
        ...
        lat_diff = haversine_distance(gnss_lat, 0, route_coordinates[waypoint][0], 0)
        long_diff = haversine_distance(0, gnss_long, 0, route_coordinates[waypoint][1])
        ...
        if distanceToNextWaypoint <= waypoint_tolerance:
            ...
            print(f"Lat difference: {lat_diff * 3.281:.2f}ft Long differencce: {long_diff * 3.281:.2f}ft")
