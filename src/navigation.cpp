...
void NavigationTask(void *pvParameters) {
    ...
    LOG_NAV("NavigationTask started, running at %d Hz", NAV_UPDATE_FREQUENCY);
    // Task loop
    while (true) {
        ...
        LOG_NAV("NavigationTask time, %lu", millis() - start);
        ...
        if (xSemaphoreTake(navDataMutex, pdMS_TO_TICKS(mutexWait)) == pdTRUE) {
            ...
            if (followingWaypoints) {
                ...
                if (targetDistance > 0 && distanceTraveled >= targetDistance) {
                    ...
                    LOG_NAV("Target distance of %.2f m reached (traveled: %.2f m)", 
                            targetDistance, distanceTraveled);
                }
            } else {
                ...
                LOG_NAV("Single destination reached, stopping navigation");
            }
